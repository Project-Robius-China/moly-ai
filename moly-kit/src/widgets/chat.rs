        ...
        for action in event.actions() {
            ...
            match action.cast::<MessagesAction>() {
                ...
                MessagesAction::QuickReply(action_text) => {
                    if let Some(url) = external_quick_reply_url(&action_text) {
...
fn external_quick_reply_url(action: &str) -> Option<&str> {
    let url = action.strip_prefix("open:")?;
...
mod tests {
    ...
    fn test_external_quick_reply() {
        ...
        assert_eq!(
            external_quick_reply_url("open:http://example.com"),
            Some("http://example.com")
        );
        assert_eq!(external_quick_reply_url("cb:hello"), None);
        assert_eq!(external_quick_reply_url("https://example.com"), None);
        assert_eq!(external_quick_reply_url("hello"), None);
    }
}
