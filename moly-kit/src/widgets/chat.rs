        ...
        for action in event.actions() {
            ...
            match action.cast::<MessagesAction>() {
                ...
                MessagesAction::QuickReply(action_text) => {
                    if is_external_quick_reply_action(&action_text) {
                        let _ = robius_open::Uri::new(action_text.as_str()).open();
                        continue;
                    }
...
fn is_external_quick_reply_action(action: &str) -> bool {
    action.starts_with("https://") || action.starts_with("http://")
}
...
mod tests {
    ...
    fn test_is_external_quick_reply_action() {
        assert!(is_external_quick_reply_action("https://example.com"));
        assert!(is_external_quick_reply_action("http://example.com"));
        assert!(!is_external_quick_reply_action("cb:hello"));
        assert!(!is_external_quick_reply_action("hello"));
    }
}
